...
/* //! Crucial Info !!!!
In this demonstration, we will intentionally make the victim invoke a swap transaction 
from WETH to USDC in UniswapV2 with 0 minimum amount. What makes this transaction 
vulnerable to a sandwich attack is the minimum amount that was set to 0, which means 
the transaction will not revert, even if the victim only receives 0 USDC or 99% slippage. 
That’s why it’s very crucial to set the minimum amount properly.
 */

/* //! Crucial Info !!!!
An attacker and victim started with 1000 WETH as the initial balance, and the attacker managed to 
frontrun and backrun the victim transaction, 
